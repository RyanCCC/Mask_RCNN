...
class ParallelModel(KM.Model):
    ...
    It works by creating a copy of the model on each GPU. Then it slices the inputs and 
    sends a slice to eack copy of the model, and then merges the outpus together and 
    applies the loss on the combined outputs.
    '''

    def __init__(self, mask_rcnn_model, gpu_count):
        '''
        Class constructor
        mask rcnn model: The model to patallelize
        gpu_count: Number of GPUS, must be >1
        '''
        self.inner_model = mask_rcnn_model
        self.gpu_count = gpu_count
        merge_outputs = self.make_parallel()
        super(ParallelModel, self).__init__(inputs=self.inner_model.inputs, outputs=merge_outputs)
